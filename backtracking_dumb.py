...
def backtracking_dumb(puzzle):    #gameObject puzzle
    ...
    while puzzle_arr.length():
        ...
        if (utility.isGoal(cur_puzzle)):
            ...
            if (utility.in_board(top_pos, cur_puzzle.configuration) && !utility.occupied(top_pos, cur_puzzle.configuration)):
                tmp.append(top_pos)
            if (utility.in_board(left_pos, cur_puzzle.configuration) && !utility.occupied(left_pos, cur_puzzle.configuration)):
                tmp.append(left_pos) 
            if (utility.in_board(right_pos, cur_puzzle.configuration) && !utility.occupied(right_pos, cur_puzzle.configuration)):
                tmp.append(right_pos)
            if (utility.in_board(bottom_pos, cur_puzzle.configuration) && !utility.occupied(bottom_pos, cur_puzzle.configuration)):
            ...
            for pos in tmp:
                if (cur_puzzle.is_source(pos, color)):
                    if (cur_puzzle.exact_n_surrounding(color, pos, cur_puzzle.configuration, 2)):
                        new_puzzle = new puzzle(cur_puzzle.configuration, cur_puzzle.color_dic, cur_puzzle.blank_pos)
...
def main():
    ...
    p = new puzzle(ret_matrix, color_position, empty_position)
